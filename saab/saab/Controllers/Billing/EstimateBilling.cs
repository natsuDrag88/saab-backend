...
{
    [Route("api/v1/facturas/estimacion")]
    [ApiController]
    public class EstimateBilling : ControllerBase
    {
        private readonly IBillingService _billingService;

        public EstimateBilling(IBillingService billingService)
        {
        ...
        [HttpGet]
        [SwaggerOperation(
            Summary = "Información de estimación de facturas.",
            Description = "Información de estimación de facturas por periodo anual o mensual de todos los proyectos.",
            OperationId = "Get",
        ...
        public Task<ObjectResult> Get([BindRequired, FromQuery] string periodo)
        {
            try
            {
                ...
                {
                    var estimateBilling = _billingService.GetEstimateToInvoiceTotal(periodo);
                    HttpContext.Response.StatusCode = 200;
                    return Task.FromResult(StatusCode(StatusCodes.Status200OK, JsonConvert.SerializeObject(estimateBilling)));
                }
                else
                {
            ...
            {
                HttpContext.Response.StatusCode = 400;
                var resultDictionary = ResponseDictionary.GetDictionaryError400(description: MessagesRequest.ErrorPeriod, error: e.Message);
                return Task.FromResult(StatusCode(StatusCodes.Status400BadRequest, resultDictionary));
            }
        }
